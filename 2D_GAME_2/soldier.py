...
class Soldier():
    ...
    def __init__(self):
        ...
        self.go_defence_font = load_font('font_file\ENCR10B.TTF', 80)
        self.game_over_font = load_font('font_file\CARTOON.ttf', 100)
        ...
        self.hp_rd_image = load_image('png_file\soldier_hp_rad.png')
    ...
    def draw(self):
        if self.game_state == self.GAME_DEFENCE:
            ...
            if self.in_go_defence():
                self.go_defence_font.draw(100, 500, 'BUTTON SHIFT', (255, 0, 0))
                self.go_defence_font.draw(50, 400, 'return defence', (255, 0, 0))

        self.ship_home_image.draw(self.home_x, self.home_y - self.space.frame)

        if self.game_state == self.GAME_OVER:
            self.game_over_font.draw(100, 500, 'GAME OVER!!', (255, 0, 0))