...
class Alien():
    ...
    def shot_area(self, soldier):
        rdr = math.pow(soldier.x - self.x, 2) + math.pow(soldier.y - self.draw_y, 2)

        if self.reload <= 2:
            return False
        else: self.reload = 0

        if math.sqrt(rdr) <= 200:
            return True
        else:
            return False
    ...
    def __init__(self):
        ...
        self.arrow_font = load_font('font_file\ENCR10B.TTF', 13)

        self.down_arrow_image = load_image('png_file\_arrow_down.png')
        self.up_arrow_image = load_image('png_file\_arrow_up.png')
    ...
    def draw(self):
        if self.state == self.LIFE:
            ...
            if self.draw_y > 1025:
                value = (self.draw_y - 1025) / self.PIXEL_PER_METTER
                self.up_arrow_image.draw(self.x, 985)
                self.arrow_font.draw(self.x - 9, 960, '%d' %value, (255, 0, 0))

            if self.draw_y < -25:
                value = ((-25) - self.draw_y) / self.PIXEL_PER_METTER
                self.down_arrow_image.draw(self.x, 25)
                self.arrow_font.draw(self.x - 8, 50, '%d' %value, (255, 0, 0))