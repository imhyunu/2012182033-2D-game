...
def enter():
    ...
    alien_bullet_team = [Alien_Bullet() for i in range(100)]
    tower = creat_tower()
...
def update(frame_time):
    ...
    for alien in alien_team:
        if alien.shot_area(soldier):
            for bullet in alien_bullet_team:
                if bullet.what_state():
                    bullet.shot(soldier, alien)
                    break
